	...
	public static function import( $customizer_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			ESS_Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing customizer settings', 'everse-starter-sites' )
			);
		}
		else {
			// Add this message to log file.
			$log_added = ESS_Helpers::append_to_file(
				esc_html__( 'Customizer settings import finished!', 'everse-starter-sites' ),
				$log_file_path,
				esc_html__( 'Importing customizer settings' , 'everse-starter-sites' )
			);
		}
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'everse-starter-sites' ),
					$import_file_path
				)
			);
		}
		...
		// Get the upload data.
		$raw = ESS_Helpers::data_from_file( $import_file_path );
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'everse-starter-sites' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'everse-starter-sites' )
			);
		}

		// Import images.
		if ( apply_filters( 'everse-starter-sites/customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			require ESS_DIR_PATH . 'inc/class-ess-customizer-option.php';

			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new ESS_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
		...
		$use_wp_customize_save_hooks = apply_filters( 'everse-starter-sites/enable_wp_customize_save_hooks', false );
